...
public class Main {
	...
	public static String nameOfTest = "Book1";
...
/*	public static void main(String[] args) throws UnirestException {
	...
	public static void main(String[] args) throws ClientProtocolException, IOException, JSONException, UnirestException, SQLException {
		...
		Map<Integer, Collection<Object[]>> answer = new HashMap<>();
		
		answer.put(1, new ArrayList<Object[]>());
		answer.put(2, new ArrayList<Object[]>());
		answer.put(3, new ArrayList<Object[]>());
		answer.put(4, new ArrayList<Object[]>());		
		...
		for (int i = 1; i < allRows.size(); i++) {
			...
			Map<Integer, Integer> ans = findAnswerQuestion(Content, A, B, C, D, isCover);
			
			System.out.println("    Ans:      " + ans + 
					         "\n    Is cover: " + isCover.toString());
			
			answer.get(1).add(new String[] {ID, ans.get(1).toString(), isCover.get(1).toString()});
			answer.get(2).add(new String[] {ID, ans.get(2).toString(), isCover.get(2).toString()});
			answer.get(3).add(new String[] {ID, ans.get(3).toString(), isCover.get(3).toString()});
			answer.get(4).add(new String[] {ID, ans.get(4).toString(), isCover.get(4).toString()});
		}
		
		for (int i = 1; i <= 4; i++) {
		
			String pathToAnswerFile = "H:/Bo_De/" + nameOfTest + "-Answer-" + i + ".csv";
			...
			writer.writeRowsAndClose(answer.get(i));
		}
	...
	private static Map<Integer, Integer> findAnswerQuestion(String Content, String A, String B, String C, String D, Map<Integer, IsCover> isCover) throws SQLException, UnirestException {
		...
		for (int n = 1; n <= 4; n++) {
			...
			else if ("B".equals(finalAnswer)) {
				resultMap.put(n, 1);
			}
			else if ("C".equals(finalAnswer)) {
				resultMap.put(n, 2);
			}
			else {
				resultMap.put(n, 3);
			...
			isCover.put(n, new IsCover().setTrue(cover.get(finalAnswer)));
		}
